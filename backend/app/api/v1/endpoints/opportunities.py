...
async def create_opportunity(
    ...
    opp = await OpportunityService(db).create_opportunity(opp_in, current_user, ws_ctx=ws_ctx)
...
async def get_pipeline_board(
    ...
    return await OpportunityService(db).get_pipeline_board(current_user, ws_ctx=ws_ctx, owner_id=owner_id)
...
async def get_opportunity(
    ...
    opp = await OpportunityService(db).get_opportunity(opportunity_id, current_user, ws_ctx=ws_ctx)
    return OpportunityRead.model_validate(opp)
...
async def update_opportunity(
    ...
    opp = await OpportunityService(db).update_opportunity(opportunity_id, opp_in, current_user, ws_ctx=ws_ctx)
    return OpportunityRead.model_validate(opp)
...
async def update_opportunity_stage(
    ...
    opp = await OpportunityService(db).update_stage(opportunity_id, stage_in, current_user, ws_ctx=ws_ctx)
    return OpportunityRead.model_validate(opp)
...
async def delete_opportunity(
    ...
    await OpportunityService(db).delete_opportunity(opportunity_id, current_user, ws_ctx=ws_ctx)
...
async def get_opportunity_activities(
    ...
    activities = await ActivityService(db).get_timeline(current_user=current_user, ws_ctx=ws_ctx, opportunity_id=opportunity_id)
...
async def analyze_opportunity_deal(
    opportunity_id: uuid.UUID,
    db: DBSession,
    current_user: CurrentActiveUser,
    ws_ctx: WorkspaceContextDep,
    ...
    opp = await OpportunityService(db).get_opportunity(opportunity_id, current_user, ws_ctx=ws_ctx)
    activities = await ActivityService(db).get_timeline(current_user=current_user, ws_ctx=ws_ctx, opportunity_id=opportunity_id, limit=5)
    ...
    next_action = (
        action_data.get("recommended_action")
        or risk_data.get("next_best_action")
        or "Follow up with prospect"
    )

    return OpportunityRiskAnalysis(
        opportunity_id=str(opp.id),
        deal_name=opp.name,
        risk_level=str(risk_data.get("risk_level") or "Medium"),
        risk_factors=list(risk_data.get("risk_factors") or []),
        recommendations=list(risk_data.get("recommendations") or []),
        next_best_action=str(next_action),
        conversion_probability=int(risk_data.get("conversion_probability") or opp.probability or 50),
    )
